...
Mirrors the manual audit steps used throughout the PolyCrit project:
  * duplicate detection via a normalized-tuple hash across the fields that
    define "the same underlying (polymer_backbone, recipe) condition"
  * contradiction detection: rows that share that identity but disagree on
    reached_lccc / interaction_parameter_c

Note this is a second line of defense, not the primary mechanism: the
extraction prompt (prompts.py) is now responsible for not fragmenting one
condition across architecture variants in the first place. This module
mainly catches genuine re-statements of the same condition (e.g. from
overlapping PDF chunks) and real disagreements worth a human's attention.
"""
...
# condition (used for duplicate detection). A condition is a
# (polymer_backbone, recipe) pair -- see prompts.py -- so identity is keyed
# on polymer_backbone plus the recipe fields (solvents, ratio, temperature),
# NOT on the free-text analyte_polymer/architecture description (which
# legitimately varies in wording even for the same backbone+recipe) and NOT
# on measured_value (two rows for the true same condition should collapse
# to one regardless of which piece of evidence -- e.g. which Mn point --
# produced them).
IDENTITY_FIELDS = [
    "polymer_backbone",
