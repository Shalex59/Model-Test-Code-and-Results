...
CONDITION_FIELDS = [
    ("sample_id", "Sample ID"),
    ("polymer_backbone", "Polymer Backbone"),
...
CONDITION_FIELD_DESCRIPTIONS = {
    "sample_id": "A short stable identifier for THIS CRITICAL/NON-CRITICAL CONDITION -- i.e. this one (polymer_backbone, mobile phase recipe) pair -- not for an individual experimental run or architecture variant. If the paper tests five different architectures at the same recipe, that is ONE condition and ONE sample_id, not five.",
    "polymer_backbone": "The canonical repeat-unit chemistry this condition applies to, e.g. 'PLA', 'PEG', 'PMMA', 'PS-b-PMMA (PS block)'. Architecture variants of the SAME backbone -- linear, star, branched, graft, end-functionalized, different arm counts -- are the SAME polymer_backbone and must be described by ONE condition row, not one row each. Only give two rows the same polymer_backbone value AND separate rows if the paper reports a genuinely different mobile-phase recipe (different solvent ratio, solvent system, or temperature) for that backbone.",
    "analyte_polymer": "The polymer class/name analyzed (e.g. 'poly(lactide)'). Do not encode architecture here in a way that implies a new condition -- see architecture_note.",
    "critical_component": "The chemical unit whose critical adsorption point is being probed -- normally the repeating-unit backbone named in polymer_backbone.",
    "architecture_note": "Free-text summary of which architecture(s) this ONE condition was established with and/or subsequently confirmed across (e.g. 'established via linear reference; mobile phase then held fixed while testing 1-6 arm star architectures with core/end functionalization'). This field absorbs architecture variety -- it does NOT trigger separate rows. If the paper reports how architecture/functional groups shift NET retention at this fixed recipe, summarize that finding here or in notes, not as new condition rows.",
    ...
    "reached_lccc": "One of: 'Yes', 'No', 'Borderline/Near-critical', 'Unclear'. Classifies this polymer_backbone's behavior UNDER THIS ONE RECIPE, not any single architecture's behavior. "
                    "'Yes' only if the paper's own evidence supports a true critical point for this backbone at this recipe (e.g. elution independent of molar mass). "
                    ...
                    "Do NOT set this to 'No' just because one architecture variant of this backbone drifts off-critical at this recipe due to added functional groups -- that is an architecture effect (record it in architecture_note/notes), not a failure of the recipe for the backbone.",
    "interaction_parameter_c": "'0' for true LCCC, '>0' for adsorption/LAC-dominant, '<0' for exclusion/SEC-dominant, or '' if the paper gives no basis to assign a sign, evaluated for polymer_backbone at THIS recipe. "
                               ...
                               "Never assign this per architecture variant -- it describes the backbone+recipe pair this row represents.",
    "c_meaning": "Plain-language explanation of the interaction_parameter_c call for this specific (polymer_backbone, recipe) row, referencing what in the data supports it. If the paper also reports architecture/functional-group effects layered on top of this recipe, summarize that here as context, not as a reason to change the classification.",
    ...
    return {
        ...
        "properties": {
            ...
            "conditions": {
                "type": "array",
                "description": (
                    "One entry per distinct (polymer_backbone, mobile-phase recipe) pair "
                    "the paper reports as critical (c=0) or non-critical (c>0 / c<0). "
                    "A 'recipe' is a specific solvent system + ratio + temperature + "
                    "column -- NOT an architecture. If the paper tests several "
                    "architectures (linear, star, different arm counts, functionalized "
                    "or not) of the SAME backbone at the SAME recipe, that is ONE row, "
                    "not one per architecture -- summarize the architecture range in "
                    "that row's architecture_note/notes fields instead. Only create "
                    "additional rows when the paper reports a genuinely different "
                    "recipe (different solvent ratio, solvent system, or temperature) "
                    "for a backbone, with a single specific numeric value -- never a "
                    "range or 'various'."
                ),
